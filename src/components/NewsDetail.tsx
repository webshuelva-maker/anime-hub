...
export function NewsDetail({
  ...
  return (
    <AnimatePresence>
      {item && (
        <motion.div
          className="fixed inset-0 z-30 flex items-start justify-center overflow-y-auto bg-black/70 p-4 py-10 backdrop-blur-sm sm:items-center"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          >
            <div className="relative">
              <NewsCover category={item.category} relatedTitle={item.relatedTitle} />
              <button
                type="button"
                onClick={onClose}
                aria-label="Cerrar"
                className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full border border-white/25 bg-black/40 text-white backdrop-blur-sm transition-transform hover:scale-110"
              >
                ✕
              </button>
            </div>

            <div className="max-h-[65vh] overflow-y-auto scrollbar-thin p-6 sm:p-8">
